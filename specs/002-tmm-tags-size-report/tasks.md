...
- [ ] T049 [P] In `public/tools/tmm/index.html`, verify the empty canvas state: when `initiatives[]` is empty and no filter is active, the centred hint "Load a matrix or click [+ Quick Add] to get started" MUST be visible over the canvas (FR-UX-001); `#inbox-panel` shows `[▶ 0]` by default. Verify the hint is conditionally shown/hidden by `renderCanvas()` based on `initiatives.filter(i => i.tmm.x !== null).length === 0` — not just present in the HTML as a static element
